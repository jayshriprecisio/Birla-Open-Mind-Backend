...
const listAdmissionInquiriesRepo = async (args) => {
  ...
  if (args.grade && args.grade.toUpperCase() !== 'ALL') {
    where['$grade_ref.name$'] = args.grade; // Or short_form depending on requirements
  }
  ...
  // Fetch unique schools and grades for the filters
...
const createAdmissionInquiryRepo = async (args) => {
  // Resolve IDs (keeping the same logic as before but using Models)
