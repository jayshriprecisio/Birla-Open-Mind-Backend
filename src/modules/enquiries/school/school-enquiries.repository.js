...
const findAdmissionInquiryByPhoneRepo = async (phone) => {
  // We'll use raw query here because of the complex joins and regex filtering which are easier in SQL
  const sql = `
    SELECT
      ai.id::text AS id,
...
const createSchoolEnquiryRepo = async ({ userId, payload }) => {
  ...
  try {
    ...
    // Handle sibling_id identity check for manual sequence management if necessary
