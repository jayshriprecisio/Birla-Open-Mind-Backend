...
const normalizeBody = (body: Record<string, unknown>) => {
  const rawEnv =
    typeof body.environment === 'string' ? body.environment.trim().toUpperCase() : body.environment;
  const environment = rawEnv === 'PRODUCTION' ? 'PROD' : rawEnv;
  return {
    ...body,
    environment,
...
export const createController = async (req: NextRequest) => {
  try {
    ...
    return NextResponse.json({
      success: true,
      data,
      message: 'EaseBuzz mapping created successfully',
    });
  } catch (error: unknown) {
    return NextResponse.json(
      { success: false, message: formatApiError(error) },
...
export const updateController = async (req: NextRequest) => {
  try {
    ...
    return NextResponse.json({
      success: true,
      data,
      message: 'EaseBuzz mapping updated successfully',
    });
  } catch (error: unknown) {
    return NextResponse.json(
      { success: false, message: formatApiError(error) },
...
export const deleteController = async (req: NextRequest) => {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'EaseBuzz mapping deleted successfully',
    });
  } catch (error: unknown) {
    return NextResponse.json(
      { success: false, message: formatApiError(error) },
