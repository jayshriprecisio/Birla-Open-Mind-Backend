...
import type { ListSchoolsQuery } from './schools.validation';
...
export type CreateSchoolInput = {
  ...
  billing_same_as_school: boolean;
  ...
  cbse_school_code: string | null;
  udise_code: string | null;
  status: string;
  /** JWT user id when UUID-shaped; otherwise omitted in INSERT as NULL */
  ...
  /** Omitted for brand BOMPS (Centre Head only). */
  principal?: {
    full_name: string;
    email_login_id: string;
    phone_number: string;
...
export async function createSchoolTransaction(
  ...
  try {
    ...
    const schoolRes = await client.query<{ school_id: string }>(
      `
      INSERT INTO schools (
    ...
    const isBomps =
      (input.brand_code ?? '').trim().toUpperCase() === 'BOMPS';
...
/**
 * Join zone_master/schools.zone_id when FK column types differ (e.g. bigint id vs varchar zone_id).
 * Avoids: operator does not exist: integer = character varying
 */
const JOIN_ZONE_SQL = `z.id::text = NULLIF(TRIM(BOTH FROM COALESCE(s.zone_id::text, '')), '')`;

/** Same pattern for brand_master.id ↔ schools.brand_id type mismatches. */
...
const SORT_SQL: Record<ListSchoolsQuery['sortBy'], string> = {
...
/**
 * Substring search via position(lower(needle) in lower(col))) avoids ILIKE wildcard injection.
 * Combine OR branches so one index-friendly pattern per column is possible later (e.g. pg_trgm).
 */
function buildSchoolListFilters(params: ListSchoolsQuery): {
  whereSql: string;
  values: unknown[];
} {
...
export async function listSchoolsFilteredRepo(params: ListSchoolsQuery): Promise<{
  rows: Record<string, unknown>[];
  total: number;
}> {
  const { whereSql, values } = buildSchoolListFilters(params);
  const orderCol = SORT_SQL[params.sortBy];
  const orderDir = params.sortOrder.toUpperCase() === 'ASC' ? 'ASC' : 'DESC';
...
export type SchoolsDashboardSummary = {
  total: number;
  bomis: number;
  bomps: number;
  /** Both UDISE and CBSE school codes present. */
  mapped: number;
  /** Schools with operational status `active`. */
  active: number;
};

export async function getSchoolsDashboardSummaryRepo(): Promise<SchoolsDashboardSummary> {
...
export async function getSchoolByIdRepo(schoolId: string) {
...
export async function softDeleteSchoolRepo(schoolId: string) {
  return pool.query(
    `
    UPDATE schools
...
export async function patchSchoolStatusRepo(
  schoolId: string,
...
export async function updateSchoolTransaction(
  schoolId: string,
  input: CreateSchoolInput
) {
  const client = await pool.connect();
